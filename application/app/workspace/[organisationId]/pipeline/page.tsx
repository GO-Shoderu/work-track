...
export default async function ManagedPipelinePage({ params }: { params: Promise<{ organisationId: string }> }) {
  ...
  return (
    <WorkspaceShell title={organisation.name} name={profile.full_name} context={`Managing Customer Workspace: ${organisation.name}`} workspaceBasePath={workspaceBasePath} currentSection="pipeline">
      ...
      <RecruitmentWorkspace organisationId={organisation.id} view="pipeline" />
    </WorkspaceShell>
  );
}
