...
export default async function ManagedCandidatesPage({ params }: { params: Promise<{ organisationId: string }> }) {
  ...
  return (
    <WorkspaceShell title={organisation.name} name={profile.full_name} context={`Managing Customer Workspace: ${organisation.name}`} workspaceBasePath={workspaceBasePath} currentSection="candidates">
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
        <div className="inline-flex items-center gap-2 rounded-full border border-[#d8ef78] bg-lime-subtle px-3 py-1.5 text-xs font-semibold text-[#405300]">
          <span className="size-2 rounded-full bg-lime-hover" />Managing customer workspace
        </div>
      ...
      <RecruitmentWorkspace organisationId={organisation.id} view="candidates" />
    </WorkspaceShell>
  );
}
