...
export async function RecruitmentWorkspace({ organisationId }: { organisationId?: string }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <section id="pipeline" className="scroll-mt-6 rounded-2xl border border-border bg-surface p-5 shadow-[0_1px_2px_rgb(13_15_20/0.03)] sm:p-6">
        <div className="mb-5">
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        <section id="jobs" className="scroll-mt-6 overflow-hidden rounded-2xl border border-border bg-surface shadow-[0_1px_2px_rgb(13_15_20/0.03)]">
          <div className="border-b border-border p-5 sm:p-6">
            <p className="text-xs font-semibold uppercase tracking-[0.16em] text-muted">Jobs</p>
            <h2 className="mt-1 text-lg font-semibold">Open roles</h2>
            <CreateJobForm organisationId={organisationId} />
          </div>
          {jobs.length ? (
            <ul className="divide-y divide-border">
              {jobs.map((job) => (
                <li key={job.id} className="px-5 py-4 sm:px-6">
                  <p className="text-sm font-semibold">{job.title}</p>
                  <p className="mt-1 text-sm leading-5 text-muted">{job.description || "No job description provided."}</p>
                </li>
              ))}
            </ul>
          ) : (
            <p className="px-6 py-10 text-center text-sm text-muted">No jobs yet. Create the first role above.</p>
          )}
        </section>

        <section id="candidates" className="scroll-mt-6 overflow-hidden rounded-2xl border border-border bg-surface shadow-[0_1px_2px_rgb(13_15_20/0.03)]">
          <div className="border-b border-border p-5 sm:p-6">
            <p className="text-xs font-semibold uppercase tracking-[0.16em] text-muted">Candidates</p>
            <h2 className="mt-1 text-lg font-semibold">Talent pool</h2>
            <CreateCandidateForm organisationId={organisationId} />
          </div>
          {candidates.length ? (
            <ul className="divide-y divide-border">
              {candidates.map((candidate) => (
                <li key={candidate.id} className="flex items-start justify-between gap-4 px-5 py-4 sm:px-6">
            ...
            <p className="px-6 py-10 text-center text-sm text-muted">No candidates yet. Add the first candidate above.</p>
          )}
        </section>
      </div>
        ...
        <div className="grid gap-5 lg:grid-cols-[minmax(0,0.8fr)_minmax(0,1.2fr)] lg:items-center">
          <div>
            ...
            <p className="mt-2 max-w-lg text-sm leading-6 text-gray-400">
              Link an existing candidate to a job. New applications start in Applied and can move through the board.
            </p>
          </div>
          <CreateApplicationForm
            organisationId={organisationId}
            candidates={candidates.map((candidate) => ({ id: candidate.id, name: candidate.full_name }))}
            jobs={jobs.map((job) => ({ id: job.id, title: job.title }))}
          />
        </div>
      </section>
